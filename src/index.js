...
<div class="subtitle">NIFTY 50 • Live Option Chain</div>
...
async function loadData() {
  ...
  try {
    ...
    view.textContent = analysis.view;
    ...
    document.getElementById("callOI").textContent =
      Number(analysis.total_call_oi || 0).toLocaleString("en-IN");

    document.getElementById("putOI").textContent =
      Number(analysis.total_put_oi || 0).toLocaleString("en-IN");
...
async function getOptions(request, env) {
  ...
  try {
    ...
    /*
      If expiry is supplied:
      /api/options?expiry=2026-09-01

      Otherwise use the next Tuesday.
    */
    ...
    for (const row of chain) {
      ...
      const putOI =
        row.put_options?.market_data?.oi || 0;
    ...
    for (const candidate of chain) {
      ...
      const strike = candidate.strike_price;

      let pain = 0;

      for (const row of chain) {

        const rowStrike = row.strike_price;
        ...
        if (rowStrike > strike) {
          pain += (rowStrike - strike) * callOI;
        }

        if (rowStrike < strike) {
          pain += (strike - rowStrike) * putOI;
        }
    ...
    const options = nearby.map(row => {
      ...
      return {
        ...
        call: {
          ltp: call.market_data?.ltp ?? null,
          oi: call.market_data?.oi ?? null,
          volume: call.market_data?.volume ?? null,
          iv: call.option_greeks?.iv ?? null,
          delta: call.option_greeks?.delta ?? null,
          theta: call.option_greeks?.theta ?? null
        },

        put: {
          ltp: put.market_data?.ltp ?? null,
          oi: put.market_data?.oi ?? null,
          volume: put.market_data?.volume ?? null,
          iv: put.option_greeks?.iv ?? null,
          delta: put.option_greeks?.delta ?? null,
          theta: put.option_greeks?.theta ?? null
        }
